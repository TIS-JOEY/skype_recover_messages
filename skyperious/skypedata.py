...
class SkypeDatabase(object):
    ...
    def get_message(self):
      sql = "SELECT id, convo_id, timestamp, type, author, body_xml FROM Messages ORDER BY timestamp DESC LIMIT 100;"
      result = self.execute(sql)
      import xml.etree.ElementTree as ET
      #doc=et.fromstring(xmlData)
      for row in result:
        if row==None or "partlist" not in row['body_xml']:
          print(row['body_xml'])
    ...
    def renew_messages(self,renew_messages):
        print(str(renew_messages['id']))
        sql = "UPDATE Messages SET body_xml = '"+renew_messages['body_xml']+"' WHERE id = "+str(renew_messages['id'])
        print(sql)
        cursor = self.execute(str(sql))
        self.connection.commit()
        print('done')
